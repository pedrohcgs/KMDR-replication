...
*5. Now sorting according to month and locating first 3 consecutive months
 where individual is working, defined as esr=1|2, ws1amt>0, and selfemp~=2
 Get rid of those with less than three months work history;
gen mark=0;
replace mark=1 if (esr==1|esr==2)&ws1amt>=0&ws1amt~=.&selfemp~=2;
...
*Match only by suid and pnum -- broad definition;
...
rename pnum pnsp;
replace pnsp = pnsp-1000*int(pnsp/1000);
...
drop temp1 temp2 temp3 temp4 mark count;

collapse (mean) qearn ws1qearn fqinc spqinc qwks, by (suid pnum qtr);
...
keep suid pnum qtr fanninc spanninc qearn_l* ws1qearn_l* qwks_l* bpw hq1w hq2w annwg cv_earn;
save temp, replace;
use ~/bulk/unemp$Y, clear;
sort suid pnum qtr;
