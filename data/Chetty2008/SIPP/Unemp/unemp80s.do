...
*fixing fields so no weird stuff
for ws1amt ws1uhours uiamt suppuiamt wksjb\ -9 -9 -9 -9 -9, ltype(any): replace @1=0 if @1==@2
...
*5. Now sorting according to month and locating first 3 consecutive months
 where individual is working, defined as esr=1|2, ws1amt>0, and selfemp~=2 (ignore last cond here)
...
*education;
replace higrade=higrade-1 if grd_cmpl==2&higrade>0 ;
gen ed=higrade;
...
gen fulltime = ws1uhours[_n-1]>35;
